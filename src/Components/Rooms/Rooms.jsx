...
const Rooms = () => {
  ...
  const handleFavoriteClick = (roomId, isFavorite) => {
    ...
    if (userId) {
      dispatch(toggleFavorite({ roomId, userId, isFavorite }));
    } else {
      setAuthOpen(true);
    }
  };
  ...
  return (
    <div className="Rooms">
            ...
            filteredRooms.map((room) => (
              <div key={room.id} className="card">
                ...
                <div className="reviews">
                  ...
                  <h3 onClick={() => handleFavoriteClick(room.id, room.isFavorite)}>
      ...
      {authOpen && <Auth isOpen={authOpen} onClose={() => setAuthOpen(false)} />}
    </div>
  );
};
